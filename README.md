...
- ✅ Per-type wire-format decoders for **35 stateless types + 1 stateful (`0x81 CvaRawPpg`)**, 0% decode errors. On the 70 h `sunday_evening.log` test capture (117,662 inner records, 34 distinct types):
  - **80.9% strong-decode** (95,145 records — typed semantic fields, cross-validated against the on-device DB where applicable):
  ...
  - **2.2% hex-only** (2,555 records — auto-extracted parser produces only opaque
    bytes; lib has no field-level structure for these): `0x6a SleepPeriodInfo2`
    (2,177), `0x75 SleepTemp` (315), `0x79 TagEvent` (60), `0x5c UserInfo` (3)
  - **17.0% no-decoder** (19,962 records): `0x61 DEBUG_DATA` (19,645 — 46 sub-types per RTTI, sub-dispatcher pending) + 2 unmapped firmware-extension tags `0x33` (240) / `0x85` (77)
...
```
## Verification — 193 claims tested
   PASS=190  FAIL=3
```
...
- 🟡 `0x61 API_DEBUG_DATA` (46 sub-types per RTTI) emits raw hex — needs a
  sub-dispatcher. **This is the single biggest gap**: 19,645 records / 16.7%
  of the stream. Mostly diagnostic, but at this volume worth a sub-byte
  histogram + targeted disassembly of the 5-10 dominant sub-types.
